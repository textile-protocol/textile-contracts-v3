...
/// @notice Linked library: order policy and constructor checks. Kept out of
...
library VaultPolicy {
    ...
    returns (bytes4)
  {
    ...
    uint256 quotableS;
    uint256 quotableC;
    if (inputToken == settlement) quotableS = src.quotableSettlement();
    else if (inputToken == corridor) quotableC = src.quotableCorridor();
    if (
      !_orderPolicyOk(
        order,
        OrderContext({
