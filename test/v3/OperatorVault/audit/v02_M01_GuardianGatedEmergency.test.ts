...
import { closeRedeemAsAnyone, seedShares } from '../helpers/vaultLifecycle'
...
describe('audit v0.2 M-01 — emergency exit no longer needs the guardian', function () {
  it('unwinds a full-supply redeem with every operator key silent', async function () {
    ...
    // Nobody with an operator key closes either (L-02 made that the
    // operator's call); a third party does once the grace has run.
    const epochId = await closeRedeemAsAnyone(ctx)
  ...
  it('keeps unwinding later epochs through the same paused path', async function () {
    ...
    const second = await closeRedeemAsAnyone(ctx)
    await time.increase(timeout)
